...
1.  **DocGenerator**: Acts as the orchestrator. It resolves file paths (supporting multi-root workspaces), loads source and context files, renders Handlebars templates for prompts, and manages the file system I/O for the resulting documentation.
...
- **Loading**: Reading source files and global context files (e.g., README.md).
- **Preparation**: Reading existing documentation or generating a skeleton if the file is new.
- **Prompting**: Combining all data into a system instruction and a user prompt using Handlebars templates.
- **Generation**: Sending the prompt to the AI and receiving a processed Markdown response.
- **Persistence**: Writing the cleaned Markdown back to the workspace.

## API Reference

### GitContext [source](../src/core/docGenerator.ts)
...
### DocGenerator [source](../src/core/docGenerator.ts)
...
### GeminiClient [source](../src/core/geminiClient.ts)
